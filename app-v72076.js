...
const APP_VERSION = "7.20.76-X3-NESTED-PRO-463-IOS-UPDATE-GUARD";
const APP_DISPLAY_VERSION = "V7.20.76 • X3 Nested Pro 463 • iPhone Update Guard Pro";
const APP_BUILD_TAG = "72076iosupdateguard";
...
// V7.20.76 — iPhone/PWA Update Guard Pro.
...
async function fetchPublishedBuildV72076(){
  ...
  // Compatibility fallback: lets V7.20.76 still detect deployments that forgot version.json.
  try{
...
async function forcePublishedBuildV72076(published){
...
async function checkForPublishedBuildV72076(force=false){
  ...
  try{
    const published=await fetchPublishedBuildV72076();
    return await forcePublishedBuildV72076(published);
  ...
  window.addEventListener("load",()=>{
    const updatePwaShell=async()=>{
      try{
        ...
        if(reg.waiting) try{ reg.waiting.postMessage({type:"SKIP_WAITING"}); }catch(_){}
        await checkForPublishedBuildV72076(true);
      }catch(_){}
    };
...
window.addEventListener("pageshow",()=>{ checkForPublishedBuildV72076(false); },{passive:true});
document.addEventListener("visibilitychange",()=>{
  if(document.visibilityState==="visible") checkForPublishedBuildV72076(false);
},{passive:true});
