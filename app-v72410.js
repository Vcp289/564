...
const APP_DISPLAY_VERSION = "V7.24.10 • Fast Strict Refresh Profile Pro";
const APP_BUILD_TAG = "72410adaptiverefreshprofilepro";
...
async function rebuildWalkForwardExactActualRow(profileId, actualDrawId, options={}) {
  ...
  // V7.24.10: profile-wide manual refresh may rebuild many exact rows. Keep the
...
// V7.24.10 — Fast Strict Refresh Profile.
  ...
  try{
    ...
    for(let i=0;i<draws.length;i+=HISTORY_PROFILE_REFRESH_PUBLISH_BATCH){
      ...
      for(const draw of batch){
        ...
        if(atomic?.complete) repaired++; else skipped++;
        processed++;
      }
      if(label){
