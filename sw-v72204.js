...
self.addEventListener("fetch",event=>{
  ...
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72204.json':
        url.pathname.endsWith('style-v72204.css')?'./style-v72204.css':
        url.pathname.endsWith('pro-core-v72204.js')?'./pro-core-v72204.js':
        url.pathname.endsWith('app-v72204.js')?'./app-v72204.js':'./x3-pro-v72204.js');
      if(hit) return hit;
