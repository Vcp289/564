...
const APP_VERSION = "7.20.80-X3-NESTED-PRO-463-AI-REALTIME-STATUS-PRO";
const APP_DISPLAY_VERSION = "V7.20.80 • X3 Nested Pro 463 • AI Realtime Status Pro";
const APP_BUILD_TAG = "72080airealtimestatuspro";
...
// V7.20.80 — REALTIME AI RESULT BADGES.
// Daily selection remains locked; only WAITING/HIT/REV/MISS is re-hydrated from live History.
function refreshAISelectLiveStatusPanel(){
  if(state.currentView!=="weekly") return false;
  const current=document.querySelector("main.main .ai-select-top3-card");
  if(!current) return false;
  const tpl=document.createElement("template");
  ...
  if(!next) return false;
  current.replaceWith(next);
  try{ bindAISelectHistoryLinks?.(next); }catch(_){}
  return true;
}
function notifyLiveHistoryMutation(){
  invalidateViewCache();
  try{ window.dispatchEvent(new CustomEvent("lucky:history-mutated")); }catch(_){
    try{ window.dispatchEvent(new Event("lucky:history-mutated")); }catch(__){}
  }
  if(state.currentView==="weekly") refreshAISelectLiveStatusPanel();
}
window.addEventListener("lucky:history-mutated",()=>{
  if(state.currentView==="weekly") refreshAISelectLiveStatusPanel();
});
document.addEventListener("visibilitychange",()=>{
  if(!document.hidden && state.currentView==="weekly") refreshAISelectLiveStatusPanel();
},{passive:true});
...
// V7.20.80 — iPhone/PWA Update Guard Pro + AI Trend Boot Lock + Strict Prior AUTO Route Daily Lock.
