const BUILD = "72206emergencyunlock";
...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72206\.css|pro-core-v72206\.js|app-v72206\.js|x3-pro-v72206\.js|manifest-v72206\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72206.json':
        url.pathname.endsWith('style-v72206.css')?'./style-v72206.css':
        url.pathname.endsWith('pro-core-v72206.js')?'./pro-core-v72206.js':
        url.pathname.endsWith('app-v72206.js')?'./app-v72206.js':'./x3-pro-v72206.js');
      if(hit) return hit;
