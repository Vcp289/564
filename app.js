...
// V4.15: สร้าง History ผล L อัตโนมัติจากเลขออกจริง + ตารางของ Profile/วันที่เดียวกัน
...
function syncAutoLHistoryForActual(actualDraw) {
  ...
  const table = getDailyTable(profileId, actualDraw.date);
...
function syncAutoLHistoryForDate(profileId, date) {
  state.actualDraws
    .filter(x => Number(x.profileId ?? 0) === Number(profileId) && x.date === date)
...
function saveDailyTableForm() {
  ...
  document.getElementById("confirmDailyTable").addEventListener("click", () => {
    ...
    syncAutoLHistoryForDate(state.activeProfile, date);
...
function openDailyTableDetail(id) {
  ...
  document.getElementById("saveDailyTableDate").addEventListener("click", () => {
    ...
    syncAutoLHistoryForDate(t.profileId, oldDate);
    syncAutoLHistoryForDate(t.profileId, newDate);
    saveState();
    closeModal();
    render();
  });
...
function renderHistory() {
  ...
  const rows = state.records
    .filter(r => r.profileId === state.activeProfile && r.status !== "notfound")
    .sort((a,b) => b.date.localeCompare(a.date) || b.createdAt - a.createdAt)
    .map(r => `<article class="history-item" data-record="${r.id}">
      <div><small>${formatDateTH(r.date)} • ${DAYS_TH[new Date(`${r.date}T12:00:00`).getDay()]}${r.autoGenerated ? " • Auto Match" : ""}</small><h3>${r.status === "notfound" ? escapeHtml(r.actualResult) : `${escapeHtml(r.selectedNumber || "-")} → ${escapeHtml(r.actualResult)}`}</h3><p>${r.status === "notfound" ? "Not Found ในชุด L" : `${escapeHtml(r.patternId || "-")} • ${escapeHtml(r.patternName || "-")} • Actual ${escapeHtml(r.actualResult || "-")}`}</p></div>
  ...
  <section class="card"><div class="section-head"><h2>Historyผล L 3 ตัว</h2><span>${state.records.filter(r=>r.profileId===state.activeProfile && r.status!=="notfound").length} รายการ</span></div>${profileTabs()}<div class="history-list">${rows || `<div class="empty-card flat">ยังไม่มีรายการที่ Match กับเลข L</div>`}</div></section>`;
}
...
function openActualDrawForm(existingId = null) {
  ...
  document.getElementById("btnSaveActualDraw").addEventListener("click", () => {
    ...
    if (existing) {
      ...
      savedActual = { id: uid(), profileId, profileName, date, number, note, createdAt: Date.now() };
