...
function renderHistory() {
  ...
  const resultRows = [...selectedActualDraws]
    .sort((a,b) => b.date.localeCompare(a.date) || (b.createdAt || 0) - (a.createdAt || 0))
    .map(r => {
      const table = getDailyTable(selectedProfile, r.date);
...
function renderAnalysis() {
  ...
  const linkedDraws = draws.filter(d => getDailyTable(profileId, d.date));
  ...
  return `<section class="card">
    ...
    <div class="analysis-source-note">ใช้ผลย้อนหลังทุกงวดเป็นฐานคำนวณ และนับ History L เฉพาะรายการที่ Match</div>
