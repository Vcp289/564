const BUILD = "72080airealtimestatuspro";
...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72080\.css|pro-core-v72080\.js|app-v72080\.js|x3-pro-v72080\.js|manifest-v72080\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72080.json':
        url.pathname.endsWith('style-v72080.css')?'./style-v72080.css':
        url.pathname.endsWith('pro-core-v72080.js')?'./pro-core-v72080.js':
        url.pathname.endsWith('app-v72080.js')?'./app-v72080.js':'./x3-pro-v72080.js');
      if(hit) return hit;
