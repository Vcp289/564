...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72077\.css|pro-core-v72077\.js|app-v72077\.js|x3-pro-v72077\.js|manifest-v72077\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72077.json':
        url.pathname.endsWith('style-v72077.css')?'./style-v72077.css':
        url.pathname.endsWith('pro-core-v72077.js')?'./pro-core-v72077.js':
        url.pathname.endsWith('app-v72077.js')?'./app-v72077.js':'./x3-pro-v72077.js');
      if(hit) return hit;
