const BUILD = "72076iosupdateguard";
...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72076\.css|pro-core-v72076\.js|app-v72076\.js|x3-pro-v72076\.js|manifest-v72076\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72076.json':
        url.pathname.endsWith('style-v72076.css')?'./style-v72076.css':
        url.pathname.endsWith('pro-core-v72076.js')?'./pro-core-v72076.js':
        url.pathname.endsWith('app-v72076.js')?'./app-v72076.js':'./x3-pro-v72076.js');
      if(hit) return hit;
