const BUILD = "72091unifiedhistorymutation";
...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72091\.css|pro-core-v72091\.js|app-v72091\.js|x3-pro-v72091\.js|manifest-v72091\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72091.json':
        url.pathname.endsWith('style-v72091.css')?'./style-v72091.css':
        url.pathname.endsWith('pro-core-v72091.js')?'./pro-core-v72091.js':
        url.pathname.endsWith('app-v72091.js')?'./app-v72091.js':'./x3-pro-v72091.js');
      if(hit) return hit;
