const BUILD = "72086tdurabledelete";
...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72086t\.css|pro-core-v72086t\.js|app-v72086t\.js|x3-pro-v72086t\.js|manifest-v72086t\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72086t.json':
        url.pathname.endsWith('style-v72086t.css')?'./style-v72086t.css':
        url.pathname.endsWith('pro-core-v72086t.js')?'./pro-core-v72086t.js':
        url.pathname.endsWith('app-v72086t.js')?'./app-v72086t.js':'./x3-pro-v72086t.js');
      if(hit) return hit;
