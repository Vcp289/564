...
const APP_DISPLAY_VERSION = "V7.24.07 • Refresh Profile Pro";
const APP_BUILD_TAG = "72407refreshprofilepro";
...
async function rebuildWalkForwardExactActualRow(profileId, actualDrawId, options={}) {
  ...
  // V7.24.07: profile-wide manual refresh may rebuild many exact rows. Keep the
...
// V7.24.07 — Manual Refresh Profile for History.
// Explicit user action only: never starts from navigation, startup, profile switching,
// or ordinary History render. It repairs the CURRENT profile in chronological order so
// each repaired row can seed the next strict-prior source. Continuous Save remains untouched.
...
function historyProfileRowNeedsRepair(draw,profileId){
  if(getAtomicHistoryStatuses(draw,profileId)) return false;
...
function strictPriorTableForHistoryDraw(profileId,draw){
  try{
    const table=getPredictionTable(profileId,draw?.date,draw);
    return table && isStrictPriorReferenceTable(table,String(draw?.date||'').slice(0,10),profileId) ? table : null;
  }catch(_){ return null; }
}

async function refreshEntireHistoryProfile(){
  if(historyProfileRefreshRunning) return;
  const id=Number(state.activeProfile)||0;
  ...
  historyProfileRefreshRunning=true;
  ...
  try{
    const draws=(state.actualDraws||[])
      .filter(d=>Number(d?.profileId??0)===id && /^\d{3}$/.test(String(d?.number||'')) && /^\d{4}-\d{2}-\d{2}$/.test(String(d?.date||'')))
      .sort((a,b)=>String(a.date).localeCompare(String(b.date))||Number(a.createdAt||0)-Number(b.createdAt||0));
    const missing=draws.filter(d=>historyProfileRowNeedsRepair(d,id));
    if(!missing.length){
      if(label) label.textContent='✓ Up to date';
      setTimeout(()=>{ const b=document.getElementById('btnHistoryRefreshProfile'); const l=b?.querySelector('.history-profile-refresh-label'); if(l) l.textContent='Refresh Profile'; b?.classList.remove('working'); },1100);
      return;
    }

    let repaired=0, unavailable=0, attempted=0, sinceYield=0;
    // Seed strict-prior continuity from the most recent actual row before the first gap.
    const firstMissingDate=String(missing[0]?.date||'');
    const prior=[...draws].filter(d=>String(d?.date||'')<firstMissingDate).pop();
    if(prior){ try{ prepareNextHistoryPredictionLock(prior); }catch(_){} }

    for(const draw of missing){
      if(historyProfileRefreshCancelRequested) break;
      attempted++;
      if(label) label.textContent=`${attempted}/${missing.length}`;
      if(icon) icon.textContent='↻';

      let table=strictPriorTableForHistoryDraw(id,draw);
      if(!table){
        // Try only the immediately preceding known result as a source seed. This never uses
        // the target row's result to predict itself, preserving strict-prior methodology.
        const idx=draws.findIndex(d=>String(d?.id||'')===String(draw?.id||''));
        const previous=idx>0?draws[idx-1]:null;
        if(previous){ try{ prepareNextHistoryPredictionLock(previous); }catch(_){} }
        table=strictPriorTableForHistoryDraw(id,draw);
      }
      if(!table){ unavailable++; continue; }
      ...
      try{
        ...
        if(atomic?.complete){
          repaired++;
          patchHistoryRowStatusesInstant(id,String(draw.id||''),{atomicOnly:true});
          // Prepare the next strict-prior source while this chronological chain is valid.
          try{ prepareNextHistoryPredictionLock(draw); }catch(_){}
        } else unavailable++;
      }catch(error){
        unavailable++;
        console.warn('Refresh Profile row skipped',draw?.date,error);
      }

      sinceYield++;
      // Keep Safari responsive and thermally bounded during an explicit whole-profile repair.
    ...
    if(repaired){
      // One global invalidation + one durable commit for the entire manual operation.
      clearPerformanceCaches(); activeRenderPerfSignature=''; invalidateViewCache();
    ...
    if(label){
      if(historyProfileRefreshCancelRequested) label.textContent=`Stopped • ${repaired}`;
      else if(repaired) label.textContent=unavailable?`✓ ${repaired} • — ${unavailable}`:`✓ ${repaired}`;
      else label.textContent=unavailable?'No prior source':'Up to date';
    }
    setTimeout(()=>{
      const b=document.getElementById('btnHistoryRefreshProfile');
      const l=b?.querySelector('.history-profile-refresh-label');
      if(l) l.textContent='Refresh Profile';
