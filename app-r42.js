...
// V7.20.46 — a normal newest-draw Save temporarily makes a previously complete WF
// bucket one row shorter than History. That is a VALID incremental prefix, not corruption.
// History first-paint must preserve it until the background append transaction extends it.
function walkForwardBucketIsStrictPrefixOfCurrentHistory(profileId, bucket=getWalkForwardBucket(profileId)) {
  const id=Number(profileId), draws=walkForwardProfileDraws(id);
  const records=Array.isArray(bucket?.records)?bucket.records:[];
  if(!bucket || Number(bucket.version||0)<4 || String(bucket.engineVersion||"")!==WF_ENGINE_VERSION) return false;
  if(String(bucket.methodology||"")!=="walk-forward-adaptive-memory-prior-only") return false;
  // V7.20.47: append-safe means EXACTLY one newest History row is missing.
  // A badly shrunken bucket such as 1/109 must never be accepted as a healthy prefix.
  if(!records.length || records.length !== draws.length - 1) return false;
  // Append-safe only: every existing WF record must still match the same History prefix.
  for(let i=0;i<records.length;i++){
    const row=records[i], draw=draws[i];
    if(!row || !draw) return false;
    if(Number(row.profileId)!==id) return false;
...
function scheduleWalkForwardAppendResume(profileId, delay=220) {
  const id=Number(profileId);
  if(!Number.isInteger(id) || id<0 || id>=state.profiles.length) return false;
  if(WF_APPEND_RESUME_IN_FLIGHT.has(id)) return false;
  const bucket=getWalkForwardBucket(id), draws=walkForwardProfileDraws(id);
  if(!walkForwardBucketIsStrictPrefixOfCurrentHistory(id,bucket)) return false;
  const records=Array.isArray(bucket?.records)?bucket.records:[];
  const missing=draws[records.length];
  if(!missing || !/^\d{4}-\d{2}-\d{2}$/.test(String(missing.date||""))) return false;
  WF_APPEND_RESUME_IN_FLIGHT.add(id);
  const run=async()=>{
    try {
      // Avoid competing with an authoritative restore/rebuild worker or the Save transaction.
      if(backgroundWfWorkerRunning){ setTimeout(run,700); return; }
      const liveBucket=getWalkForwardBucket(id);
      if(walkForwardBucketCoversCurrentHistory(id,liveBucket)) return;
      if(!walkForwardBucketIsStrictPrefixOfCurrentHistory(id,liveBucket)) return;
      const liveDraws=walkForwardProfileDraws(id), liveRecords=Array.isArray(liveBucket?.records)?liveBucket.records:[];
      const nextDraw=liveDraws[liveRecords.length];
      if(!nextDraw) return;
      ...
      if(document.visibilityState!=="hidden") setTimeout(()=>render(),60);
      console.info(`WF append resume complete: ${state.profiles[id]||`Profile ${id+1}`} (${liveRecords.length} -> ${liveDraws.length})`);
    } catch(error) {
      console.error("WF append resume failed", state.profiles[id]||id, error);
    } finally {
      WF_APPEND_RESUME_IN_FLIGHT.delete(id);
    }
  };
...
function ensureProfileDerivedHistoryReady(profileId, {repairTables=true} = {}) {
  ...
  if (draws.length >= 8) {
    ...
    if (!wfReady) {
      const appendPrefix = Boolean(bucket && walkForwardBucketIsStrictPrefixOfCurrentHistory(id, bucket));
      if (appendPrefix) {
        // Preserve N trusted rows and resume ONLY the missing N+1 row. This also recovers
        // automatically if iOS suspended the Save background task before it could append.
        wfQueued = scheduleWalkForwardAppendResume(id, 180);
      } else {
...
function scheduleHistorySummaryCacheBuild(profileId, draws, visibleSummaries=null){
  ...
  setTimeout(async()=>{
    try{
      ...
      // Publish only a COMPLETE + NON-SHRINKING generation. V7.20.45 guarded the explicit
      // mutation transaction, but this idle summary builder was a second publication path that
      // could still overwrite a good N-row snapshot with a transient 1-row generation.
      if(pending===0){
        const rowFingerprints={}, rowMeta={};
        for(const draw of list){
          const key=unifiedAIRowKey(draw);
          if(rows[key]){
            rowFingerprints[key]=historySnapshotRowFingerprint(draw);
            rowMeta[key]={trusted:true};
          }
        }
        const candidate={ok:true,trusted,pending:0,rows,rowFingerprints,rowMeta,summaries,generation:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`};
        const lastCommitted=readLastCommittedAIHistorySnapshot(id);
        const guard=committedSnapshotReplacementGuard(lastCommitted,candidate,list,{previousCoverage:committedSnapshotRowCount(lastCommitted)});
        if(guard.ok){
          persistHistorySummaryCache(id,list,summaries);
          persistCommittedAIHistorySnapshot(id,list,candidate);
          ...
          // Keep the proven generation visible and let the canonical transaction/repair finish.
          scheduleAIHistoryCoverageRepair(id,900);
        }
