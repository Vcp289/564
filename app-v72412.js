...
const APP_VERSION = "7.24.12-FRESH-WF-REFRESH-PRO";
const APP_DISPLAY_VERSION = "V7.24.12 • Fresh WF Refresh Pro";
const APP_BUILD_TAG = "72412adaptiverefreshprofilepro";
...
async function rebuildWalkForwardBacktest(profileId, progressCallback = null, options = {}) {
  ...
  if(!requestedStartDate && startIndex===0){
    // V7.24.12: manual Refresh Profile must never resume an old/poisoned WF checkpoint.
    // A checkpoint can be structurally valid while containing pending rows from an older
    // History engine, which makes a "completed" rebuild preserve dashes forever.
    if(forceFresh){ try{ await deleteIndexedValue(progressKey); }catch(_){} }
    const checkpoint=forceFresh ? null : await readIndexedValue(progressKey);
...
async function rebuildWalkForwardExactActualRow(profileId, actualDrawId, options={}) {
  ...
  // V7.24.12: profile-wide manual refresh may rebuild many exact rows. Keep the
...
function getAtomicHistoryStatuses(draw,profileId=Number(draw?.profileId??0)){
  ...
  // V7.24.12 — publish per engine. One unavailable engine must never hide the
  // strict-prior result of the other five. "complete" remains diagnostic only.
  // Return null only when no engine has a publishable strict-prior status at all.
  const resolved=keys.filter(k=>['exact','reversed','swap','notfound','miss'].includes(String(st[k]||'pending').toLowerCase()));
    ...
    ${profileTabs()}
      ...
      <div class="history-manager-panel history-ref-1">
        ...
        <div class="history-table-toolbar">
          ...
          ${selectedActualDraws.length ? `<div class="history-toolbar-actions"><button type="button" id="btnHistoryRefreshProfile" class="history-profile-refresh-btn" aria-label="Refresh current History profile"><span class="history-profile-refresh-icon">↻</span><span class="history-profile-refresh-label">Refresh</span></button><button type="button" id="btnHistoryEdit" class="history-edit-toggle${historyEditMode ? " active" : ""}">${historyEditMode ? "Done" : "Edit"}</button></div>` : ""}
        </div>
...
function bindView() {
  ...
  if (state.currentView === "history") {
    ...
    document.getElementById("btnHistoryRefreshProfile")?.addEventListener("click", async event => {
      event.preventDefault(); event.stopPropagation();
      if(historyProfileRefreshRunning){
        historyProfileRefreshCancelRequested=true;
        const label=event.currentTarget?.querySelector?.('.history-profile-refresh-label');
        if(label) label.textContent='Stopping…';
        return;
      }
      await refreshEntireHistoryProfile();
    });
...
// V7.24.12 — Fast Strict Refresh Profile.
// Explicit user action only. Continuous Save remains untouched.
// Instead of rebuilding every missing row independently (which repeatedly rebuilds the
// same AI memory), run ONE O(n) strict-prior walk-forward pass for this Profile, then
// publish atomic six-engine History statuses in light batches. This is both faster and
// cooler while preserving the exact prior-only boundary: every target row uses only an
// earlier reference table / earlier actual results.
const HISTORY_PROFILE_REFRESH_PUBLISH_BATCH=18;
let historyProfileRefreshRunning=false;
let historyProfileRefreshCancelRequested=false;

function historyProfileRowNeedsRepair(draw,profileId){
  const route=getHistoryRouteAStatuses(draw,profileId,{display:false});
  return ['classic','aiL','gl','p18','p19','x3'].some(k=>String(route?.[k]||'pending').toLowerCase()==='pending');
}

function profileRefreshDraws(profileId){
  const id=Number(profileId);
  return (state.actualDraws||[])
    .filter(d=>Number(d?.profileId??0)===id && /^\d{3}$/.test(String(d?.number||'')) && /^\d{2}$/.test(String(d?.twoDigit||'')) && /^\d{4}-\d{2}-\d{2}$/.test(String(d?.date||'')))
    .sort((a,b)=>String(a.date).localeCompare(String(b.date))||Number(a.createdAt||0)-Number(b.createdAt||0));
...
function prepareStrictPriorReferenceChainForProfile(profileId,draws){
  const id=Number(profileId); let prepared=0;
  for(const draw of (draws||[])){
    if(Number(draw?.profileId??0)!==id) continue;
    const existing=getDailyTable(id,String(draw.date||''));
    if(existing?.inputDigits?.length===5) continue;
    try{ if(upsertDailyTableFromActual(draw)) prepared++; }catch(error){ console.warn('Refresh Profile reference prepare skipped',draw?.date,error); }
  }
  return prepared;
}

async function refreshEntireHistoryProfile(){
  if(historyProfileRefreshRunning){ historyProfileRefreshCancelRequested=true; return; }
  const id=Number(state.activeProfile)||0;
  const button=document.getElementById('btnHistoryRefreshProfile');
  const label=button?.querySelector('.history-profile-refresh-label');
  const icon=button?.querySelector('.history-profile-refresh-icon');
  historyProfileRefreshRunning=true; historyProfileRefreshCancelRequested=false;
  button?.classList.add('working'); if(label) label.textContent='0%'; if(icon) icon.textContent='↻';

  try{
    const draws=profileRefreshDraws(id);
    const initialMissing=draws.filter(d=>historyProfileRowNeedsRepair(d,id)).length;
    if(!initialMissing){
      if(label) label.textContent='✓';
      setTimeout(()=>{ const b=document.getElementById('btnHistoryRefreshProfile'),l=b?.querySelector('.history-profile-refresh-label'); if(l) l.textContent='Refresh'; b?.classList.remove('working'); },700);
      return;
    }

    prepareStrictPriorReferenceChainForProfile(id,draws);
    if(historyProfileRefreshCancelRequested) return;
    if(label) label.textContent='Build 5%';
    await new Promise(resolve=>setTimeout(resolve,0));

    let rebuiltBucket=null;
    try{
      await rebuildWalkForwardBacktest(id,(done,total)=>{
        if(historyProfileRefreshCancelRequested) return;
        if(label && total>0){
          const pct=5+Math.round((Math.min(total,done+1)/total)*70);
          label.textContent=`Build ${Math.min(75,pct)}%`;
        }
      },{ fastEvolution:true, yieldEvery:10, progressEvery:6, deferDurable:true, checkpointEvery:64, forceFresh:true });
      rebuiltBucket=getWalkForwardBucket(id)||null;
    }catch(error){ console.error('Fast Refresh Profile WF failed',error); }
    if(historyProfileRefreshCancelRequested) return;

    const byId=new Map((rebuiltBucket?.records||[]).filter(Boolean).map(r=>[String(r.actualDrawId||''),r]));
    const byDate=new Map((rebuiltBucket?.records||[]).filter(Boolean).map(r=>[String(r.date||''),r]));
    let repaired=0, skipped=0, processed=0;
    for(let i=0;i<draws.length;i+=HISTORY_PROFILE_REFRESH_PUBLISH_BATCH){
      if(historyProfileRefreshCancelRequested) break;
      const batch=draws.slice(i,i+HISTORY_PROFILE_REFRESH_PUBLISH_BATCH);
      for(const draw of batch){
        if(!historyProfileRowNeedsRepair(draw,id)) { processed++; continue; }
        const rec=byId.get(String(draw.id||''))||byDate.get(String(draw.date||''))||null;
        let atomic=null;
        try{ atomic=rec?buildAtomicHistoryStatusesForExactRow(id,draw,rec):null; }catch(error){ console.warn('Refresh Profile atomic publish skipped',draw?.date,error); }
        if(atomic && ['classic','aiL','gl','p18','p19','x3'].some(k=>['exact','reversed','swap','notfound','miss'].includes(String(atomic.statuses?.[k]||'pending').toLowerCase()))) repaired++; else skipped++;
        processed++;
      }
      if(label){
        const pct=75+Math.round((processed/Math.max(1,draws.length))*23);
        label.textContent=`Apply ${Math.min(98,pct)}%`;
      }
      await new Promise(resolve=>setTimeout(resolve,0));
    }
    ...
    notifyLiveHistoryMutation(id);
    if(state.currentView==='history' && Number(state.activeProfile)===id) refreshCurrentView();

    const remaining=draws.filter(d=>historyProfileRowNeedsRepair(d,id)).length;
    if(label){
      if(historyProfileRefreshCancelRequested) label.textContent=`Stop ${repaired}`;
      else if(!remaining) label.textContent='✓ Done';
      else if(repaired) label.textContent=`✓ ${repaired}`;
      else label.textContent='No source';
    }
    showToast(historyProfileRefreshCancelRequested
      ? `หยุด Refresh • สำเร็จ ${repaired}`
      : `Refresh Profile เสร็จ • สำเร็จ ${repaired}${remaining?` • คงเหลือ ${remaining}`:''}`);
    setTimeout(()=>{ const b=document.getElementById('btnHistoryRefreshProfile'),l=b?.querySelector('.history-profile-refresh-label'); if(l) l.textContent='Refresh'; b?.classList.remove('working'); },1200);
  } finally {
    historyProfileRefreshRunning=false; historyProfileRefreshCancelRequested=false;
  }
}
