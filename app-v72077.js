...
const APP_VERSION = "7.20.77-X3-NESTED-PRO-463-PRO-IA-FAST";
const APP_DISPLAY_VERSION = "V7.20.77 • X3 Nested Pro 463 • Pro IA Fast";
const APP_BUILD_TAG = "72077proiafast";
...
// V7.20.77 — iPhone/PWA Update Guard Pro + Pro information architecture cleanup.
...
async function forcePublishedBuildV72077(published){
...
async function checkForPublishedBuildV72077(force=false){
  ...
  try{
    const published=await fetchPublishedBuildV72077();
    return await forcePublishedBuildV72077(published);
  ...
  window.addEventListener("load",()=>{
    const updatePwaShell=async()=>{
      try{
        ...
        if(reg.waiting) try{ reg.waiting.postMessage({type:"SKIP_WAITING"}); }catch(_){}
        await checkForPublishedBuildV72077(true);
      }catch(_){}
    };
...
window.addEventListener("pageshow",()=>{ checkForPublishedBuildV72077(false); },{passive:true});
document.addEventListener("visibilitychange",()=>{
  if(document.visibilityState==="visible") checkForPublishedBuildV72077(false);
},{passive:true});
