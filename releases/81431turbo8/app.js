...
const APP_VERSION = "8.14.31.5-FAST-JSON-REBUILD-IOS";
const APP_DISPLAY_VERSION = "V8.14.31.5 • Fast JSON Rebuild iPhone";
const APP_BUILD_TAG = "81431turbo8";
...
function getHistoryRouteAStatuses(draw, profileId = Number(draw?.profileId ?? 0), options = {}) {
  ...
  // Pattern engines are allowed to resolve only when the row is trusted (LIVE/WF).
  // Their status functions themselves enforce the same strict prior-only gate.
  if(base?.trusted){
...
function patchHistoryRowStatusesInstant(profileId, drawId, options={}) {
  try {
    ...
    // V7.24.14 ROW-FIRST: never call the canonical full snapshot reader here.
    ...
    // V7.24.14 ATOMIC SAVE PAINT: the just-saved row stays as six neutral pending cells
    ...
    const labelToKey={X3:'x3',P19:'p19',P18:'p18',CLS:'classic',AIL:'aiL',GL:'gl'};
    const modelByKey={x3:'x3',p19:'p19',p18:'p18',classic:'classic',aiL:'ail',gl:'gl'};
    ...
    if(winnerCell){
      ...
      const labels={x3:'X3',p19:'P19',p18:'P18',classic:'CLS',aiL:'AIL',gl:'GL'};
...
const PERF_CACHE = {
  ...
  priorRowsBase: new Map() // PERF FIX: cached sorted base list for patternV6PriorRows (see below)
};
...
async function bootstrapPersistentState() {
  ...
  // not only during loadState(). This prevents an old 116-row IndexedDB snapshot from
...
// PERF FIX (rebuild speed): the original implementation re-filtered and re-sorted the
// ENTIRE records array on every single call. This function is invoked several times per
// row inside the Full/Turbo Rebuild loop (once per historical draw), which made a full
// rebuild O(N^2 log N) in the number of draws N — it got dramatically slower as History
// grew. The old PERF_CACHE.patternV19Evidence cache did not help because its key was
// built FROM the already-filtered/sorted result, so the expensive part still ran every
// time regardless of cache hit/miss.
//
// Fix: build the filtered+sorted "base" list once per profile and reuse it until the
// underlying WF bucket actually changes (checked via a cheap signature). Each call then
// only needs a binary search for the cutoff position + a slice, i.e. O(log N + limit)
// instead of O(N log N). Output is identical to the previous implementation.
function patternV6PriorRowsBase(profileId){
  const id=Number(profileId);
  const bucket=getWalkForwardBucket(id),records=Array.isArray(bucket?.records)?bucket.records:[];
  const last=records.length?records[records.length-1]:null;
  const sig=`${records.length}|${last?String(last.date||''):''}|${last?String(last.actualDrawId||''):''}`;
  const cacheKey=`PVRB|${id}`;
  const cached=PERF_CACHE.priorRowsBase.get(cacheKey);
  if(cached && cached.sig===sig) return cached.rows;
  const draws=new Map((state.actualDraws||[]).filter(d=>Number(d?.profileId??0)===id).map(d=>[String(d?.id||''),d]));
  const rows=records
    .filter(r=>String(r?.date||'')&&Array.isArray(r?.grids?.classic))
    .sort((a,b)=>String(a.date).localeCompare(String(b.date)))
    .map(r=>{
      const d=draws.get(String(r?.actualDrawId||''));
      return {date:String(r.date),grid:r.grids.classic,actual:/^\d{3}$/.test(String(d?.number||''))?canonical3(d.number):''};
    })
    .filter(x=>x.actual);
  PERF_CACHE.priorRowsBase.set(cacheKey,{sig,rows});
  return rows;
}
function patternV6PriorRows(profileId,targetDate='',limit=30){
  const cutoff=/^\d{4}-\d{2}-\d{2}$/.test(String(targetDate||''))?String(targetDate):'9999-12-31';
  const base=patternV6PriorRowsBase(profileId);
  // Binary search for the first row whose date is NOT strictly before cutoff.
  let lo=0,hi=base.length;
  while(lo<hi){
    const mid=(lo+hi)>>1;
    if(base[mid].date<cutoff) lo=mid+1; else hi=mid;
  }
  const end=lo,start=Math.max(0,end-limit);
...
function getProfileOrderByMode(mode = state.analysisSortMode) {
  ...
  if (mode === "ai") {
    // V7.24.14 PRO FINAL — navigation is read-only. Never compute a fresh ranking
...
// V7.24.14 HISTORY ENGINE V2 — exact-row WF writer.
...
// V7.24.14 HISTORY ATOMIC CHAIN — exact-row result cache.
...
function getAtomicHistoryStatuses(draw,profileId=Number(draw?.profileId??0)){
  ...
  // V7.24.14 Ranking Atomic Authority: only the strict-prior exact-row artifact is authoritative.
  // This prevents an arbitrary/legacy object from becoming Trusted merely because it has statuses.
...
// JSON Restore calls this lookup once per historical draw.  A repeated Array.find()
// turns a 2,800-row import into millions of comparisons on iPhone, so keep a small
// source-aware index and update it when a missing table is appended.
...
// V7.24.14 PRO SELF-HEAL — History/Analysis never require a manual Rebuild.
...
function renderHistory() {
  ...
  const resultRows = visibleActualDraws
    .map(r => {
      ...
      // V7.24.14: a manually repaired strict-prior atomic row is itself durable evidence.
...
function getRankingHistoryAuthoritySnapshot(profileId, draws) {
  // V7.24.14 — Ranking / History authority must use the SAME dataset signature.
  // Legacy V7.24.13 compared timestamps even though Refresh writes Summary first and Snapshot
  // second, so the snapshot was always a few ms newer and many Profiles collapsed to 1/8.
  // Dataset-signature attestation is the correct proof; write order is irrelevant.
  const id = Number(profileId), list = Array.isArray(draws) ? draws : [];
  ...
  // Fallback is permitted only when the currently attested History summary agrees with the
  // committed generation. This lets a stable checked generation survive adapter hydration
  // without accepting an unrelated stale snapshot.
...
function getCommittedHistoryRankingAuthority(draw, profileId, committedSnapshot = null) {
  // V7.24.14 — History-generation authority bridge.
  // A committed H35 row is persisted ONLY after getUnifiedAIHistoryStatuses() has already
  // passed the strict trusted History gate. On cold restore, do not demand that the old
  // prediction table can be reconstructed a second time: that caused 200+ checked History
  // rows to collapse to Trusted 1/8. The current dataset fingerprint / History attestation
  // is validated by getRankingHistoryAuthoritySnapshot() before this function is reached.
  ...
  // The snapshot itself is the prior-only proof envelope. New generations explicitly carry
  // strictPriorOnly=true; legacy H35 generations are accepted because this store has only ever
  // been populated from trusted History rows, never from retrospective display-only rows.
...
function getProfileAIRankScore(item, updateStatus = "pending") {
  ...
  // Bayesian-adjusted hit rate is the dominant performance signal. We keep the historical
  // 20%-hit => 100 normalization so Rank Score remains familiar on the existing 0–100 scale.
...
function publishProfileRankingAfterMutation(profileId){
  ...
  if(!rankingItemsHaveTrustedEvidence(serial)){
    // Derived engines are not fully trustworthy yet. Keep the frozen LKG generation and retry later.
...
// V7.24.14 STABLE SNAPSHOT FALLBACK — the last atomic generation remains displayable
...
function buildCommittedAIHistorySnapshot(profileId,draws){
  ...
  // V7.24.14: per-engine publication. A ready P19/X3 generation must never be discarded
...
function scheduleChunkedWalkForwardSelfHeal(profileId,delay=220){
  ...
  setTimeout(async()=>{
    try{
      ...
      // V7.24.14: one-shot only; never recursively keep CPU awake.
    }
...
async function runAIHistoryTransaction(profileId,reason='mutation',options={},controller=null){
  ...
  const job=previous.catch(()=>{}).then(async()=>{
    ...
    // Publish whatever is already verified immediately. This is the core V7.24.14 fix:
    ...
    if(snapshot.needsRepair && draws.length){
      try{
        ...
        // V7.24.14: never block History/Analysis on a full 200+ draw WF rebuild.
...
// V7.24.14 ANALYSIS SWR SELF-HEAL — repair missing exact snapshots after first paint.
...
function renderAnalysisModelPerformance(profileId = state.activeProfile){
  ...
  // V7.24.14 PRO FINAL — Analysis foreground path is snapshot-only.
...
function openLResults(searchValue = "", limit = currentLRankLimit, mode = currentLResultMode) {
  ...
  // V7.24.14 — L popup count authority. Every engine tab shows its de-duplicated
...
function historyRowNeedsManualRefresh(draw,profileId,committedSnapshot=null){
  ...
  // Caller can pass one shared snapshot. V7.24.04 re-read/scanned the whole History once
  // per row here, turning Refresh into O(n²) work on large profiles.
  const snapshot=committedSnapshot || readLatestCommittedAIHistorySnapshot(profileId);
  const row=snapshot?.rows?.[unifiedAIRowKey(draw)]||null;
  if(!row) return true;
  return ['classic','aiL','gl','p18','p19','x3'].some(k=>String(row?.[k]||'pending').toLowerCase()==='pending');
}
function buildLightHistorySummaries(profileId,draws){
  const id=Number(profileId)||0, keys=['classic','aiL','gl','p18','p19','x3'];
  const totals=Object.fromEntries(keys.map(k=>[k,0])), hits=Object.fromEntries(keys.map(k=>[k,0]));
...
async function refreshMissingHistoryForCurrentProfile(){
  ...
  try{
    ...
    // Newest-first: one tap repairs every missing row that is currently on screen. Older,
    // not-yet-loaded History remains untouched until the user loads that batch.
    for(const draw of visibleMissing){
      ...
      try{
        // Exact-row only. This function may update its small WF bucket but must not start a
        // suffix/full rebuild. Performance caches are cleared once after the whole tap.
    ...
    if(repaired){
      ...
      clearPerformanceCaches(); activeRenderPerfSignature=''; invalidateViewCache();
      // Recalculate only lightweight percentages from already-committed/atomic rows.
      // No hydrateUnifiedAIProfile, no WF rebuild, no retry timer.
...
function openActualDrawForm(existingId = null) {
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    // V7.24.14 ROW-FIRST / PERCENT-LATER.
...
async function deleteActualDrawWithSync(id, options={}) {
  ...
  try{
    ...
    // V7.20.89 PRO TARGETED REBUILD:
    // - latest-row delete: prune exactly one WF row (dependency-free)
  ...
  try {
    ...
    if(state.walkForwardRebuildJob.phase==="tables"){
      ...
      while(Number(state.walkForwardRebuildJob.tableIndex||0)<draws.length){
        ...
        // Keep the first paint responsive. Indexed lookups make this cheap, while a
        // 160-row Turbo batch guarantees progress is painted on iPhone immediately.
