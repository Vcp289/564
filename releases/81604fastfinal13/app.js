...
const LAST_PAINT_SNAPSHOT_MAX_LEN=260000;
...
const SUPPORT_AI_RUNTIME_ENABLED=false;
const MASTER_AI_PAUSED=true;
...
const PRODUCTION_AI_ENGINES=Object.freeze(["classic","aiL","gl","p18","p19","x3","x4"]);
...
const PROFILE_AI_MIN_TRUSTED_EVIDENCE=8;
...
const PROFILE_RANK_MIN_SIGNIFICANT_SAMPLES=30;
...
if(tier<2&&isViewCacheKey(key))continue;
...
legacyRecordRelinkPendingProfileIds:[],
activeFormulaByProfile:{},
formulaStrategyVersion:3,
...
let currentLResults=[];
let currentLRankLimit=0;
let currentLResultMode="l";
let currentLComboPair="classic-ai";
let currentLBetPerNumber=1;
let calculatorProfileRefreshToken=0;
let calculatorFirstPaintDeferred=false;
let independentCalculatePreviewProfile=null;
let calculatorTableViewMode="original";
...
const base=options?.display===false?getHistoryComparisonStatuses(draw,id):getHistoryDisplayComparisonStatuses(draw,id);
const out={
...
if(base?.trusted){
try{out.p18=patternV18HistoryStatus(draw,id)||'pending';}catch(_){}
try{out.p19=patternV19HistoryStatus(draw,id)||'pending';}catch(_){}
try{out.x3=x3HistoryStatus(draw,id)||'pending';}catch(_){}
try{out.x4=x4HistoryStatus(draw,id)||'pending';}catch(_){}
}
out.trusted=Boolean(base?.trusted);
out.verified=Boolean(base?.verified);
out.walkForward=Boolean(base?.walkForward);
out.legacy=Boolean(base?.legacy);
out.hasAI=Boolean(base?.hasAI);
return out;
}
...
const shell=document.querySelector(`[data-history-edit-shell="${CSS.escape(key)}"]`);
const row=shell?.querySelector('[data-actual-draw]');
const head=document.querySelector('.result-history-table .result-history-head');
...
const atomicReady=peek&&['classic','aiL','gl','p18','p19','x3','x4'].every(k=>{
const v=String(peek?.[k]||'pending').toLowerCase();
return['exact','reversed','swap','notfound','miss'].includes(v);
});
const directAtomicReady=['classic','aiL','gl','p18','p19','x3','x4'].every(k=>{const v=String(routeADirect?.[k]||'pending').toLowerCase();return['exact','reversed','swap','notfound','miss'].includes(v);});
...
const statuses={
x4:committed?.x4||routeA?.x4||'pending',
x3:committed?.x3||routeA?.x3||'pending',
p19:committed?.p19||routeA?.p19||'pending',
p18:committed?.p18||routeA?.p18||'pending',
classic:committed?.classic||routeA?.classic||comparison?.classic||'pending',
aiL:committed?.aiL||routeA?.aiL||comparison?.aiL||'pending',
gl:committed?.gl||routeA?.gl||comparison?.gl||'pending'
};
const labelToKey={X4:'x4',X3:'x3',P19:'p19',P18:'p18',CLS:'classic',AIL:'aiL',GL:'gl'};
const modelByKey={x4:'x4',x3:'x3',p19:'p19',p18:'p18',classic:'classic',aiL:'ail',gl:'gl'};
const headerCells=[...head.children].slice(2,-1);
const statusCells=[...row.querySelectorAll(':scope > .status')];
const scored=[];
headerCells.forEach((cell,index)=>{
const raw=String(cell.querySelector('b')?.textContent||cell.textContent||'').trim().toUpperCase();
const engineKey=labelToKey[raw];
const status=statuses[engineKey]||'pending';
const target=statusCells[index];
if(!target||!engineKey)return;
target.className=`status ${status} model-${modelByKey[engineKey]||'neutral'}`;
target.textContent=compactHistoryStatusLabel(status);
if(status!=='pending')scored.push({key:engineKey,status});
});
const winnerCell=row.querySelector(':scope > .formula-winner');
if(winnerCell){
const best=scored.length?Math.max(...scored.map(x=>formulaStatusScore(x.status))):0;
const winners=best>0?scored.filter(x=>formulaStatusScore(x.status)===best):[];
const labels={x4:'X4',x3:'X3',p19:'P19',p18:'P18',classic:'CLS',aiL:'AIL',gl:'GL'};
const winner=winners.length===1?labels[winners[0].key]:(winners.length>1?'TIE':'—');
const winnerModel=winners.length===1?(modelByKey[winners[0].key]||'none'):(winners.length>1?'tie':'none');
winnerCell.className=`formula-winner winner-${winnerModel}`;
winnerCell.textContent=winner;
}
const day=DAYS_SHORT[new Date(`${draw.date}T12:00:00`).getDay()];
const dateSmall=row.querySelector('.result-date small');
if(dateSmall)dateSmall.textContent=`${day}${comparison?.legacy?' • LEG':(comparison?.walkForward?' • WF':' • ✓')}`;
row.classList.toggle('legacy-unverified',Boolean(comparison?.legacy));
row.classList.toggle('walk-forward-prediction',Boolean(comparison?.walkForward));
row.classList.toggle('verified-prediction',Boolean(comparison?.verified));
return Object.values(statuses).some(x=>x!=='pending');
}catch(e){console.warn('Instant History status hydration skipped',e);}
return false;
}
...
calculatorEngine:new Map(),
recentAIWinner:new Map(),
profileRankingPage:new Map()
};
let activeRenderPerfSignature="";
const WF_BOOTSTRAP_IN_FLIGHT=new Set();
const WF_BOOTSTRAP_RETRY_COUNT=new Map();
...
try{localStorage.setItem(WF_BOOTSTRAP_RETRY_KEY,JSON.stringify(store));}catch(_){}
}
...
return finalizeLoadedState(replayHistoryRowJournal(main));
}
}
}catch(_){
}
...
return finalizeLoadedState(replayHistoryRowJournal(selected?.data||null));
}catch{
...
if(!indexMap.has(oldId))continue;
...
const oldExtrasFallback=extras||snapshot;
...
const engineSig=key==='x4'?(globalThis.X4Coverage577?.VERSION||'X4'):key==='x3'?X3_ENGINE_SIGNATURE:key==='p19'?PATTERN_V19_ENGINE_SIGNATURE:key==='pattern'?'P18':key;
const cacheKey=[
...
const nums=items.slice(0,5).map(x=>String(x?.number||'').padStart(3,'0')).filter(x=>/^\d{3}$/.test(x));
const grid=nums.length===5?[0,1,2].map(pos=>nums.map(n=>Number(n[pos]))):null;
return remember({key:'x4',label:'X4',grid,status:historical?'PRIOR-ONLY':(x4?.selectorStatus||'COVERAGE'),active,results:items,historical,tableKind:'coverage',targetDate:calcTargetDate,prediction:x4});
}
return null;
}
function getCalculatorEngineTablesForKeys(profileId=state.activeProfile,keys=[]){
const wanted=[...new Set((Array.isArray(keys)?keys:[]).map(String).filter(k=>['original','ai','gl','pattern','p19','x3','x4'].includes(k)))];
...
function canonicalTripleFast(a,b,c){
...
const canonical=canonicalTripleFast(value[0],value[1],value[2]);
const canBeExact=value===canonical;
for(let i=0;i<WF_L_CELL_PATHS.length;i++){
const path=WF_L_CELL_PATHS[i];
const key=canonicalTripleFast(flat[path[0]],flat[path[1]],flat[path[2]]);
...
const rows=p19x3Rows(list,id,draw=>getPredictionTable(id,draw.date,draw),false);let step;
while(!(step=rows.next()).done){const i=step.value;
...
const{total,classicWin,v18Win,v19Win,changed,gained,lost,x3Hit,rescueHits,expertHist,v18Hist,p19StatusMap,x3StatusMap,x3SelectedMap}=step.value;
const statusMap=p19StatusMap;
...
function*p19x3Rows(list,id,resolveTable,wantX3){
...
const table=resolveTable(draw,i),inputs=table?.inputDigits;
...
const x3=wantX3?buildX3FromP19Pack(p19Like,pack,{profileId:id,targetDate,inputDigits:inputs,historical:true}):{items:[]};
...
return{total,classicWin,v18Win,v19Win,changed,gained,lost,x3Hit,rescueHits,expertHist,v18Hist,p19StatusMap,x3StatusMap,x3SelectedMap};
}
...
const resolveTable=typeof options?.resolveTable==="function"?options.resolveTable:createWalkForwardTableResolver(id,list);
const rows=p19x3Rows(list,id,resolveTable,true);let step;
while(!(step=rows.next()).done){const i=step.value;if((i+1)%chunkSize===0||i===list.length-1){if(!fast&&userInteractionHot(850))await waitForForegroundIdle(850);await nextUiFrame(fast?0:2);}}
const{total,classicWin,v18Win,v19Win,changed,gained,lost,x3Hit,rescueHits,expertHist,v18Hist,p19StatusMap,x3StatusMap,x3SelectedMap}=step.value;
const rate=n=>total?Math.round(n*10000/total)/100:0,rel=(n,d)=>d?Math.round(((n/d)-1)*10000)/100:0;
const targetClassicWins=classicWin+1,targetV18Wins=Math.ceil(v18Win*(1+PATTERN_V19_TARGET_V18_RELATIVE));
...
const rows=p19x3Rows(list,id,draw=>getPredictionTable(id,draw.date,draw),true);let step;
...
let HISTORY_BACKGROUND_AUTO_REFRESH_TIMER=null;
function scheduleHistoryBackgroundAutoRefresh(profileIds=[],reason='restore-progress'){
if(state.currentView!=="history"||document.visibilityState==="hidden")return false;
const activeId=Number(state.activeProfile);
const ids=[...new Set((Array.isArray(profileIds)?profileIds:[profileIds]).map(Number).filter(Number.isFinite))];
if(ids.length&&!ids.includes(activeId))return false;
if(HISTORY_BACKGROUND_AUTO_REFRESH_TIMER)return false;
HISTORY_BACKGROUND_AUTO_REFRESH_TIMER=setTimeout(()=>{
HISTORY_BACKGROUND_AUTO_REFRESH_TIMER=null;
if(state.currentView!=="history"||document.visibilityState==="hidden")return;
const savedScroll=Math.max(0,Number(window.scrollY||document.documentElement.scrollTop||0));
const changed=refreshCurrentViewIfDataChanged(reason);
if(changed)requestAnimationFrame(()=>window.scrollTo(0,savedScroll));
},280);
return true;
}
...
const shortFor=key=>key==="x4"?"X4":key==="x3"?"X3":key==="p19"?"P19":key==="pattern"?"P18":key==="gl"?"GL":key==="ai"?"AIL":"CLS";
if(mode==="combo"){
...
return['original','ai','gl','pattern','p19','x3','x4'].includes(baseMode)?baseMode:'original';
}
...
const issues=[];
if(!['original','ai','gl','pattern','p19','x3','x4'].includes(tableKey))issues.push(`unsupported-table:${tableKey}`);
...
if(mode==="blend"&&saved?.formula)return saved.formula;
if(mode==="combo"){
...
p18:snap.p18Items||[],p19:snap.p19Items||[],x3:snap.x3Items||[],x4:snap.x4Items||[]
};
...
if(latestRemaining&&latestRemaining>=deletedDate)return false;
...
if(!table||!isStrictPriorReferenceTable(table,targetDate,id))return null;
const inputs=Array.isArray(table.inputDigits)?table.inputDigits.map(String):[];
if(inputs.length!==5||inputs.some(v=>!/^[0-9]$/.test(v)))return null;
const actual=String(draw.number||'');if(!/^\d{3}$/.test(actual))return null;
const classicGrid=formulaGrid(inputs,getOriginalFormula());if(!classicGrid)return null;
const statusFromItems=(items,computed=true)=>{
...
const a=draw?.historyAtomicStatuses;if(!a||Number(a.profileId)!==Number(profileId))return null;
if(String(a.methodology||'')!=='strict-prior-exact-row')return null;
const target=String(draw?.date||'').slice(0,10),source=String(a.sourceTableDate||'').slice(0,10);
...
function analysisWinWindowSafe(fallback=30){
const n=Number(state.analysisWinWindow);
return[0,1,7,14,30,60,90].includes(n)?n:fallback;
}
...
return`${PRO_VIEW_SNAPSHOT_SCHEMA}|analysis|p${id}|${base}|sort:${state.analysisSortMode||"ai"}|order:${state.profileOrderMode||"default"}|win:${analysisWinWindowSafe()}|l:${Number(state.analysisLWindow)||30}|show:${state.analysisLShowAll?1:0}|rw:${rc.exactPoints},${rc.weight10},${rc.weight30},${rc.weightAll}`;
}
...
if(view==="analysis")return`${view}:p${id}:w${analysisWinWindowSafe()}:s${state.analysisSortMode||"ai"}`;
return`${view}:p${id}`;
}
...
mirrorAIProfileTrendDaily(snap);
if(state.currentView==="weekly"){
...
const ENGINE_FOLLOW_MIN_SAMPLES=15;
const ENGINE_FOLLOW_MEANINGFUL_DELTA=8;
...
const ENGINE_MODE_TO_FOLLOW_KEY={original:'classic',pattern:'p18',ai:'aiL',gl:'gl',p19:'p19',x3:'x3',x4:'x4'};
function resolveSingleEngineKeyForFollow(profileId){
const mode=getActiveFormulaMode(profileId);
if(ENGINE_MODE_TO_FOLLOW_KEY[mode])return ENGINE_MODE_TO_FOLLOW_KEY[mode];
if(mode==='combo'||mode==='blend'){
const base=getAutoFormulaDecision(profileId)?.comboBaseMode;
if(ENGINE_MODE_TO_FOLLOW_KEY[base])return ENGINE_MODE_TO_FOLLOW_KEY[base];
}
return'classic';
}
...
const DAILY_TABLE_LOOKUP={source:null,length:-1,byProfileDate:new Map()};
function dailyTableLookupKey(profileId,date){return`${Number(profileId)}|${String(date||"").trim().slice(0,10)}`;}
function ensureDailyTableLookup(){
const source=Array.isArray(state.dailyTables)?state.dailyTables:[];
if(DAILY_TABLE_LOOKUP.source===source&&DAILY_TABLE_LOOKUP.length===source.length)return DAILY_TABLE_LOOKUP;
const byProfileDate=new Map();
for(const table of source){
const key=dailyTableLookupKey(table?.profileId,table?.date);
if(!byProfileDate.has(key))byProfileDate.set(key,table);
}
DAILY_TABLE_LOOKUP.source=source;DAILY_TABLE_LOOKUP.length=source.length;DAILY_TABLE_LOOKUP.byProfileDate=byProfileDate;
return DAILY_TABLE_LOOKUP;
}
function rememberDailyTableLookup(table){
const source=Array.isArray(state.dailyTables)?state.dailyTables:[];
if(DAILY_TABLE_LOOKUP.source!==source)return;
if(source.length===DAILY_TABLE_LOOKUP.length+1){
const key=dailyTableLookupKey(table?.profileId,table?.date);
if(!DAILY_TABLE_LOOKUP.byProfileDate.has(key))DAILY_TABLE_LOOKUP.byProfileDate.set(key,table);
DAILY_TABLE_LOOKUP.length=source.length;
}
}
function getDailyTable(profileId,date){
return ensureDailyTableLookup().byProfileDate.get(dailyTableLookupKey(profileId,date))||null;
}
...
let p18Results=[],p19Results=[],x3Results=[],x4Results=[];
if(classicGrid){
...
try{x4Results=(buildX4Candidates(classicGrid,profileId,targetDate,inputs,false)?.items||[]).slice();}catch(_){}
}
...
x4Items:x4Results.map(x=>String(x?.number||x)).filter(x=>/^\d{3}$/.test(x)),
canonicalSix:true,
...
else{state.dailyTables.push(payload);rememberDailyTableLookup(payload);}
return existing||payload;
}
function upsertDailyTableFromActualTurbo(actualDraw){
if(!actualDraw)return null;
const three=String(actualDraw.number||""),two=String(actualDraw.twoDigit||"");
if(!/^\d{3}$/.test(three)||!/^\d{2}$/.test(two)||!actualDraw.date)return null;
const profileId=Number(actualDraw.profileId??0),inputDigits=[...three,...two];
const formula=getOriginalFormula(),grid=formulaGrid(inputDigits,formula);
if(!grid)return null;
const existing=getDailyTable(profileId,actualDraw.date);
const payload={
id:existing?.id||uid(),profileId,
profileName:actualDraw.profileName||state.profiles[profileId]||`Profile ${profileId+1}`,
date:actualDraw.date,inputDigits,inputNumber:inputDigits.join(""),
grid:grid.map(row=>[...row]),lResults:findLResults(grid),
note:existing?.note||"สร้างจากผลวันนี้เพื่อใช้ทำนายงวดถัดไป",
autoGeneratedFromActual:true,predictionTargetDate:getNextBusinessDate(actualDraw.date),
sourceActualDrawId:actualDraw.id,createdAt:existing?.createdAt||Date.now(),updatedAt:Date.now(),
formulaMode:"original",formulaSnapshot:cloneFormula(formula)
};
if(existing)Object.assign(existing,payload);else{state.dailyTables.push(payload);rememberDailyTableLookup(payload);}
return existing||payload;
}
...
function buildHistoryChampionSummary(originalSummary,aiSummary,glSummary,independentSummary,p18Summary,p19Summary,x3Summary,masterSummary,x4Summary=null){
const candidates=[
...
...(x4Summary?.total?[{key:"x4",label:"X4",summary:x4Summary}]:[]),
...
const keys=['classic','aiL','gl','p18','p19','x3','x4'];
...
const x4Summary=resolvedHistorySummaries.x4||pendingSummary();
const enginePriority={x4:0,p19:1,x3:2,p18:3,classic:4,gl:5,aiL:6};
const engineDefs=[
{key:"x4",label:"X4",model:"x4",summary:x4Summary},
...
const comparison={classic:historyRow?.classic||'pending',aiL:historyRow?.aiL||'pending',gl:historyRow?.gl||'pending',hasAI:Boolean(historyRow?.aiL&&historyRow.aiL!=='pending'),legacy:false,walkForward:false,verified:Boolean(historyRow)};
const originalStatus=historyRow?.classic||comparison.classic||"pending";
const aiStatus=historyRow?.aiL||comparison.aiL||"pending";
const glStatus=historyRow?.gl||comparison.gl||"pending";
const p18Status=historyRow?.p18||"pending";
const p19Status=historyRow?.p19||p19BundleRow||"pending";
const x3Status=historyRow?.x3||x3BundleRow||"pending";
const x4Status=historyRow?.x4||"pending";
const day=DAYS_SHORT[new Date(`${r.date}T12:00:00`).getDay()];
const statusMap={x4:x4Status,x3:x3Status,p19:p19Status,p18:p18Status,classic:originalStatus,aiL:aiStatus,gl:glStatus};
...
return`<div class="history-edit-shell${historyEditMode?" editing":""}${deleteOpen?" delete-open":""}" data-history-edit-shell="${r.id}">
        ...
        <button class="result-history-row formula-${formulaMode}${rowWinnerClass}" data-actual-draw="${r.id}" ${pendingReason?`data-pending-reason="${escapeHtml(pendingReason)}"`:''} ${comparison.legacy?'title="Legacy: แสดงย้อนหลังเท่านั้น ไม่นับคะแนน"':(comparison.walkForward?'title="WF: Walk-Forward ใช้เฉพาะข้อมูลก่อนวันเป้าหมาย"':'title="Verified Live: มี Snapshot ก่อนผลออกจริง"')}>
          ...
          ${formulaMode==="compare"?`${engineDefs.map(x=>statusCell(statusMap[x.key],x.model)).join("")}`:""}
          ${formulaMode==="advanced"?`${engineDefs.map(x=>statusCell(statusMap[x.key],x.model)).join("")}`:""}
        </button>
    ...
    ${profileTabs()}
      ...
      <div class="history-manager-panel history-ref-1">
        ...
        <div class="history-table-toolbar">
          ...
          ${selectedActualDraws.length?`<div class="history-toolbar-actions"><button type="button" id="btnHistoryRefresh" class="history-refresh-btn" aria-label="Refresh missing History"><span class="history-refresh-icon">↻</span><span class="history-refresh-label">Refresh</span></button><button type="button" id="btnHistoryEdit" class="history-edit-toggle${historyEditMode?" active":""}">${historyEditMode?"Done":"Edit"}</button></div>`:""}
        </div>
...
function getRankingHistoryAuthoritySnapshot(profileId,draws){
const id=Number(profileId),list=Array.isArray(draws)?draws:[];
const attestation=readHistorySummaryCache(id,list);
const exact=readCommittedAIHistorySnapshot(id,list);
if(exact?.rows){
if(exact.strictPriorOnly===true||attestation?.summaries)return{...exact,rankingAuthorityMode:exact.strictPriorOnly===true?"exact-strict":"exact-attested"};
return null;
}
const latest=readLatestCommittedAIHistorySnapshot(id);
if(!latest?.rows||!attestation?.summaries)return null;
const sameSummary=JSON.stringify(latest.summaries||{})===JSON.stringify(attestation.summaries||{});
if(!sameSummary)return null;
return{...latest,rankingAuthorityMode:"history-attested"};
}
function getCommittedHistoryRankingAuthority(draw,profileId,committedSnapshot=null){
const id=Number(profileId),targetDate=String(draw?.date||"").slice(0,10);
if(!draw||!/^\d{4}-\d{2}-\d{2}$/.test(targetDate))return null;
const snapshot=committedSnapshot||null;
if(!snapshot?.rows||Number(snapshot.profileId??id)!==id)return null;
const row=snapshot.rows?.[unifiedAIRowKey(draw)]||null;
if(!row)return null;
const validStatus=value=>["exact","reversed","swap","notfound","miss"].includes(String(value||"").toLowerCase());
const classic=String(row.classic||"pending").toLowerCase();
const aiL=String(row.aiL||"pending").toLowerCase();
if(!validStatus(classic)&&!validStatus(aiL))return null;
return{row,sourceDate:"history-attested-prior",targetDate,methodology:"committed-history-trusted-generation"};
}
...
const bayesAlpha=PROFILE_BAYES_PRIOR.alpha+trustedHits;
const bayesBeta=PROFILE_BAYES_PRIOR.beta+Math.max(0,trustedSamples-trustedHits);
const bayesianRate=(bayesAlpha+bayesBeta)>0?Math.round((bayesAlpha*1000)/(bayesAlpha+bayesBeta))/10:PROFILE_BAYES_PRIOR.mean;
const bayesianStrength=trustedSamples?Math.round((trustedSamples/(trustedSamples+PROFILE_BAYES_PRIOR.strength))*100):0;
const verifiedSamples=trustedRows.filter(r=>r.source==="verified-live").length;
const atomicSamples=trustedRows.filter(r=>r.source==="strict-atomic").length;
const committedHistorySamples=trustedRows.filter(r=>r.source==="committed-history").length;
const walkForwardSamples=trustedRows.filter(r=>r.source==="walk-forward"||r.source==="strict-atomic"||r.source==="committed-history").length;
...
bayesianRate,
bayesianStrength,
bayesPriorMean:PROFILE_BAYES_PRIOR.mean,
verifiedSamples,
atomicSamples,
committedHistorySamples,
walkForwardSamples,
...
const perfNorm=Math.max(0,Math.min(100,bayesianRate*5));
...
const bayesRate=Number.isFinite(Number(item.bayesianRate))
?Number(item.bayesianRate)
:(((PROFILE_BAYES_PRIOR.alpha+Number(item.trustedHits||0))*100)/
(PROFILE_BAYES_PRIOR.strength+Number(item.trustedSamples||0)));
const adjustedHitNorm=Math.max(0,Math.min(100,bayesRate*5));
...
return Math.round((adjustedHitNorm*w.hit)+(sampleNorm*w.samples)+(confidenceNorm*w.confidence)+(freshnessNorm*w.freshness));
}
...
const existingGood=existing?.items?.length&&rankingItemsHaveTrustedEvidence(existing.items)?existing.items:null;
const authorityGood=authority?.items?.length&&rankingItemsHaveTrustedEvidence(authority.items)?authority.items:null;
const frozen=existingGood||authorityGood||bestLastKnownGoodRanking(affectedStartDate);
...
function rankingItemsHaveTrustedEvidence(items){
const list=Array.isArray(items)?items:[];
if(!list.length)return false;
return list.some(x=>Number(x?.trustedSamples||0)>0||Number(x?.verifiedSamples||0)>0||Number(x?.walkForwardSamples||0)>0||Number(x?.samples||0)>0);
}
function computePreMutationProfileAIRanking(affectedStartDate=""){
const before=/^\d{4}-\d{2}-\d{2}$/.test(String(affectedStartDate||""))?String(affectedStartDate):"";
const anchor=before?shiftIsoDate(before,-1):profileRankingTargetDate(getProfileRankingUpdateMeta());
return state.profiles.map((_,i)=>{
const options=before?{beforeDate:before,anchorDate:anchor}:{anchorDate:anchor};
const item=getProfileAIRecommendation(i,options);
return{...item,rankScore:getProfileAIRankScore(item,"pending")};
}).sort((a,b)=>
Number(b.evidenceReady)-Number(a.evidenceReady)||b.rankScore-a.rankScore||Number(b.bayesianRate||0)-Number(a.bayesianRate||0)||b.trustedSamples-a.trustedSamples||b.trustedRate-a.trustedRate||b.confidence-a.confidence||a.profileId-b.profileId
);
}
function bestLastKnownGoodRanking(affectedStartDate=""){
const mutation=readProfileRankingMutationLock();
if(mutation?.items?.length&&rankingItemsHaveTrustedEvidence(mutation.items))return mutation.items;
const rebuild=readProfileRankingRebuildLock();
if(rebuild?.items?.length&&rankingItemsHaveTrustedEvidence(rebuild.items))return rebuild.items;
const authority=readProfileRankingAuthority();
if(authority?.items?.length&&rankingItemsHaveTrustedEvidence(authority.items))return authority.items;
try{
const fallback=rankingSerializableItems(computePreMutationProfileAIRanking(affectedStartDate));
if(rankingItemsHaveTrustedEvidence(fallback))return fallback;
...
Number(b.evidenceReady)-Number(a.evidenceReady)||Number(b.rankScore)-Number(a.rankScore)||Number(b.bayesianRate||0)-Number(a.bayesianRate||0)||Number(b.trustedSamples)-Number(a.trustedSamples)||Number(b.trustedRate)-Number(a.trustedRate)||Number(b.confidence)-Number(a.confidence)||Number(a.profileId)-Number(b.profileId)
);
...
const serial=rankingSerializableItems(audit.items);
if(!rankingItemsHaveTrustedEvidence(serial)){
writeProfileRankingObject(PROFILE_RANKING_MUTATION_LOCK_KEY,{...lock,completedProfileIds:[],updatedAt:Date.now()});
throw new Error("Profile Ranking generation ยังเป็น partial/zero — คง Last-Known-Good ไว้ก่อน");
}
const snapshot={schema:PROFILE_RANKING_SCHEMA,generation:`MUT-${Date.now().toString(36)}-${sourceFingerprint.slice(0,8)}`,targetDate,sourceFingerprint,engineSignature:profileRankingEngineSignature(),publishedAt:Date.now(),digest:audit.digest,auditRuns:audit.runs,items:serial,state:"READY"};
...
return hashWalkForwardText(JSON.stringify(rankingSerializableItems(items).map((x,index)=>[index+1,x.profileId,x.rankScore,x.bayesianRate,x.trustedRate,x.trustedSamples,x.trustedHits,x.confidence,x.rankingAnchorDate])));
}
...
if(mutationLock?.state==="MUTATING"&&mutationLock?.items?.length&&rankingItemsHaveTrustedEvidence(mutationLock.items))return mutationLock.items.map(x=>({...x}));
...
if(!rankingJobIsActive()){
const snapshot={schema:PROFILE_RANKING_SCHEMA,generation:`LIVE-${sourceFingerprint.slice(0,8)}`,targetDate,sourceFingerprint,engineSignature:profileRankingEngineSignature(),publishedAt:Date.now(),digest:rankingDigest(freshSerializable),auditRuns:1,items:freshSerializable,state:"READY"};
writeProfileRankingObject(PROFILE_RANKING_AUTHORITY_KEY,snapshot);
}
return fresh;
}
...
const UNIFIED_AI_ENGINE_ORDER=Object.freeze(["classic","aiL","gl","p18","p19","x3","x4"]);
...
if(engine==="x4")return globalThis.x4HistoryStatus?.(draw,id)||"pending";
return"pending";
}
...
return{ok:true,strictPriorOnly:true,complete:pending===0,needsRepair:repairEngines.length>0,repairEngines,trusted,pending,pendingByEngine,rows,summaries,generation:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`};
}
...
const recentCacheKey=`${windowDays}|${activeRenderPerfSignature}`;
...
.sort((a,b)=>String(a.date).localeCompare(String(b.date))||Number(a.createdAt||0)-Number(b.createdAt||0));
const emptyCounts={classic:0,aiL:0,gl:0,p18:0,p19:0,x3:0,x4:0};
if(!all.length){
...
const anchorDate=windowDays===0?today
:windowDays===1?shiftIsoDate(today,-1)
:String(all.at(-1).date);
...
const startDate=windowDays===7?(sevenDrawDates?.[0]||anchorDate)
:(windowDays===0||windowDays===1)?anchorDate
:shiftIsoDate(anchorDate,-(windowDays-1));
...
const windowDays=[0,1,7,14,30,60,90].includes(Number(days))?Number(days):7;
const today=isoDate();
...
if(!all.length)return{windowDays,anchorDate:null,startDate:null,evaluated:0,tie:0,noWinner:0,counts:{...empty},profileWins:{classic:{},aiL:{},gl:{},p18:{},p19:{},x3:{},x4:{}},details:[],champion:null};
const anchorDate=windowDays===0?today:windowDays===1?shiftIsoDate(today,-1):String(all.at(-1).date),dates=[...new Set(all.map(r=>String(r.date)))].sort();
const dateSet=windowDays===7?new Set(dates.slice(-7)):null;
const startDate=windowDays===7?(dates.slice(-7)[0]||anchorDate):(windowDays===0||windowDays===1)?anchorDate:shiftIsoDate(anchorDate,-(windowDays-1));
...
const windowDays=[0,1,7,14,30,60,90].includes(Number(state.analysisWinWindow))?Number(state.analysisWinWindow):7;
...
const win=Number.isFinite(Number(details.dataset.window))&&details.dataset.window!==undefined?Number(details.dataset.window):analysisWinWindowSafe(),base=proCanonicalDataFingerprint();
...
const windowDays=[0,1,7,14,30,60,90].includes(Number(state.analysisWinWindow))?Number(state.analysisWinWindow):30;
return`<section class="card ux-page-card analysis-v690">
    ...
    ${profileTabs()}
    <div class="analysis-global-range"><span>ช่วงวิเคราะห์</span><div>${[1,0,7,14,30,60,90].map(day=>`<button type="button" class="${windowDays===day?'active':''}" data-analysis-window="${day}">${day===0?'วันนี้':day===1?'เมื่อวาน':day}</button>`).join('')}</div></div>
    ...
    <details class="ux-disclosure settings-advanced">
      ...
      <div class="ux-disclosure-body">
        <div class="settings-section-card settings-advanced-inner">
          <div class="settings-section-head"><span>🤖</span><div><b>AI Runtime</b><small>X4 + X3 + P19 + P18 + Classic L + AI L + AI GL</small></div></div>
...
document.getElementById("btnHistoryRefresh")?.addEventListener("click",async event=>{
event.preventDefault();event.stopPropagation();
await refreshMissingHistoryForCurrentProfile();
});
...
if(![0,1,7,14,30,60,90].includes(days))return;
...
openAIWinnerCalendar([0,1,7,14,30,60,90].includes(Number(state.analysisWinWindow))?Number(state.analysisWinWindow):7);
}));
...
const popupRequiredKeys=["original","ai","gl","pattern","p19","x3","x4"];
...
const x4Table=calculatorTables.find(t=>t.key==="x4")||null;
const x4Ready=Boolean(Array.isArray(x4Table?.results)&&x4Table.results.length);
const x4Ranked=x4Ready?(x4Table.results||[]).map((item,index)=>({...item,aiRank:index+1,aiScore:Number(item.score||Math.max(10,98-index*2))})):[];
const x4PopupSummary=(()=>{let hit=0,total=0;(state.actualDraws||[]).filter(d=>Number(d?.profileId??0)===Number(state.activeProfile)).forEach(draw=>{const s=x4HistoryStatus(draw,state.activeProfile);if(s==='pending')return;total++;if(s==='exact'||s==='reversed')hit++;});return{hit,total,rate:total?Math.round(hit*1000/total)/10:0};})();
const aiPerformanceOrder=[
{key:"x4",label:"X4",rate:Number(x4PopupSummary?.rate||0),items:x4Ranked,tie:0},
{key:"x3",label:"X3",rate:Number(sharedAutoDecision?.x3Rate||0),items:x3Ranked,tie:1},
...
const totalComboAvailableCount=[classicRawResults,aiLRawResults,glRawResults,patternRanked,p19Ranked,x3Ranked,x4Ranked].filter(items=>Array.isArray(items)&&items.length).length;
...
:currentLResultMode==="x4"?x4Ranked
...
:currentLResultMode==="x4"?Number(heroSummary("x4")?.total||0)
...
:currentLResultMode==="x4"?"X4 • Coverage 577"
...
?`TOTAL COMBO รวม Classic + AI L + AI GL + P18 + P19 + X3 + X4 • รวมผล → ตัดเลขซ้ำ → เลขที่ซ้ำหลายสูตรขึ้นก่อน • ใช้กฎ Consensus เดียวกับระบบ COMBO`
:currentLResultMode==="independent"
...
{label:"jsdelivr (default)",options:{}},
{label:"unpkg (fallback)",options:{
corePath:"https://unpkg.com/tesseract.js-core@5/",
workerPath:"https://unpkg.com/tesseract.js@5/dist/worker.min.js",
langPath:"https://unpkg.com/tessdata@1/tessdata"
}}
];
let workingCdnOptions=null;
for(const attempt of workerAttempts){
try{
sharedWorker=await Promise.race([
Tesseract.createWorker(["tha","eng"],1,attempt.options),
new Promise((_,reject)=>setTimeout(()=>reject(new Error("worker create timeout")),45000))
]);
await sharedWorker.setParameters({preserve_interword_spaces:"1"});
workingCdnOptions=attempt.options;
break;
}catch(workerError){
console.warn(`Shared OCR worker unavailable via ${attempt.label}`,workerError);
...
...(workingCdnOptions||workerAttempts[1].options),
logger:message=>{
...
let importRowJournalSaved=true;
for(const row of saved)importRowJournalSaved=journalHistoryRowUpsert(row)&&importRowJournalSaved;
const importInstantSaved=commitHistoryMutationInstant(state);
const importMainSaved=importInstantSaved||saveState();
...
function historyRowNeedsManualRefresh(draw,profileId,committedSnapshot=null){
const atomic=getAtomicHistoryStatuses(draw,profileId);
if(atomic?.complete)return false;
const snapshot=committedSnapshot||readLatestCommittedAIHistorySnapshot(profileId);
const row=snapshot?.rows?.[unifiedAIRowKey(draw)]||null;
if(!row)return true;
...
const id=Number(profileId)||0,keys=['classic','aiL','gl','p18','p19','x3','x4'];
const totals=Object.fromEntries(keys.map(k=>[k,0])),hits=Object.fromEntries(keys.map(k=>[k,0]));
const committed=readLatestCommittedAIHistorySnapshot(id);
for(const draw of(draws||[])){
const atomic=getAtomicHistoryStatuses(draw,id);
const row=(atomic?.complete?atomic.statuses:null)||committed?.rows?.[unifiedAIRowKey(draw)]||null;
if(!row)continue;
for(const k of keys){
const st=String(row?.[k]||'pending').toLowerCase();
if(st==='pending')continue;
totals[k]++;
if(st==='exact'||st==='reversed'||st==='swap')hits[k]++;
}
}
return Object.fromEntries(keys.map(k=>[k,{hit:hits[k],total:totals[k],rate:totals[k]?Math.round(hits[k]*1000/totals[k])/10:0}]));
}
async function refreshMissingHistoryForCurrentProfile(){
if(historyManualRefreshRunning)return;
const id=Number(state.activeProfile)||0;
const btn=document.getElementById('btnHistoryRefresh');
const label=btn?.querySelector('.history-refresh-label');
historyManualRefreshRunning=true;
btn?.classList.add('working');if(btn)btn.disabled=true;
if(label)label.textContent='Refreshing…';
try{
const draws=(state.actualDraws||[])
.filter(d=>Number(d?.profileId??0)===id&&/^\d{3}$/.test(String(d?.number||''))&&/^\d{4}-\d{2}-\d{2}$/.test(String(d?.date||'')))
.sort((a,b)=>String(b.date).localeCompare(String(a.date))||Number(b.createdAt||0)-Number(a.createdAt||0));
const committedSnapshot=readLatestCommittedAIHistorySnapshot(id);
const missing=draws.filter(d=>historyRowNeedsManualRefresh(d,id,committedSnapshot));
if(!missing.length){
if(label)label.textContent='Up to date';
setTimeout(()=>{const b=document.getElementById('btnHistoryRefresh');const l=b?.querySelector('.history-refresh-label');if(l)l.textContent='Refresh';b?.classList.remove('working');if(b)b.disabled=false;},700);
return;
}
let repaired=0,unavailable=0,attempted=0;
const visibleLimit=Math.max(HISTORY_FIRST_BATCH,Number(historyVisibleLimitByProfile[id]||HISTORY_FIRST_BATCH));
const visibleIds=new Set(draws.slice(0,visibleLimit).map(d=>String(d?.id||'')));
const visibleMissing=missing.filter(d=>visibleIds.has(String(d?.id||'')));
for(const draw of visibleMissing){
attempted++;
if(label)label.textContent=`${attempted}/${visibleMissing.length}`;
const table=getPredictionTable(id,draw.date,draw);
if(!table||!isStrictPriorReferenceTable(table,String(draw.date||'').slice(0,10),id)){
...
}catch(error){unavailable++;console.warn('Manual History row repair skipped',draw?.date,error);}
await new Promise(resolve=>setTimeout(resolve,0));
}
if(repaired){
saveState();await commitStateDurably();
clearPerformanceCaches();activeRenderPerfSignature='';invalidateViewCache();
const summaries=buildLightHistorySummaries(id,draws);
persistHistorySummaryCache(id,draws,summaries);
requestAnimationFrame(()=>{if(state.currentView==='history'&&Number(state.activeProfile)===id)refreshCurrentView();});
}
if(label)label.textContent=repaired?`✓ ${repaired}`:(unavailable?'No source':'Up to date');
setTimeout(()=>{
const b=document.getElementById('btnHistoryRefresh'),l=b?.querySelector('.history-refresh-label');
if(l)l.textContent='Refresh';b?.classList.remove('working');if(b)b.disabled=false;
},900);
}finally{
historyManualRefreshRunning=false;
}
}
...
try{
const CANONICAL_STORE_KEY='luckyNumber_canonical_engine_store_v72302';
const raw=localStorage.getItem(CANONICAL_STORE_KEY);
if(raw){
const store=JSON.parse(raw)||{};
const nextProfiles={};
Object.entries(store.profiles||{}).forEach(([oldKey,value])=>{
const oldId=Number(oldKey);
if(!indexMap.has(oldId)){nextProfiles[oldKey]=value;return;}
const newId=indexMap.get(oldId);
nextProfiles[String(newId)]={...value,profileId:newId};
});
const next={...store,profiles:nextProfiles,updatedAt:Date.now()};
localStorage.setItem(CANONICAL_STORE_KEY,JSON.stringify(next));
try{window.LNCanonicalHistory?.load?.();}catch(_){}
}
}catch(error){console.warn('Canonical history store remap skipped',error);}
try{
const nextVisible={};
...
if(job.phase==="wf"){
const rowTotal=Math.max(1,Number(job.wfRowTotal||0));
const rowFraction=rowTotal>1?Math.max(0,Math.min(1,Number(job.wfRowIndex||0)/rowTotal)):0;
const profileUnits=Number(job.wfProfileIndex||0)+rowFraction;
return Math.min(90,30+Math.floor((profileUnits/Math.max(1,wfIds.length))*60));
}
if(job.phase==="live"){
...
function rebuildTimingSummary(timings={}){
const parts=[];
for(const[key,label]of[['tablesMs','Tables'],['wfMs','WF'],['liveMs','Live'],['rankingMs','Rank']]){
const ms=Number(timings?.[key]||0);if(ms>0)parts.push(`${label} ${formatRebuildElapsed(ms)}`);
}
return parts.join(' • ');
}
...
try{
if(fastMode&&state.walkForwardRebuildJob.cleanRebuild)upsertDailyTableFromActualTurbo(draw);
else upsertDailyTableFromActual(draw);
changedProfileIds.add(Number(draw.profileId??0));
}catch(error){console.warn("Restore missing-table rebuild failed",draw.date,error);}
}
}
...
const onWfProgress=(rowIndex,rowTotal,date,meta={})=>{
const current=state.walkForwardRebuildJob;if(!current)return;
const done=Math.min(Number(rowTotal||0),Number(rowIndex||0)+1);
updateWalkForwardJob({wfRowIndex:done,wfRowTotal:Number(rowTotal||0),wfRowDate:String(date||''),lastMessage:`${fastMode?'Turbo ':''}WF ${name} • ${done}/${Number(rowTotal||0)} งวด • Profile ${idx+1}/${ids.length}`});
paintBackgroundJobProgress();
};
await rebuildWalkForwardBacktest(id,onWfProgress,fastMode
...
updateWalkForwardJob({wfProfileIndex:idx+1,wfRowIndex:0,wfRowTotal:0,wfRowDate:'',invalidProfileIds:remainingInvalid,lastMessage:`✓ WF ${name}`});
scheduleHistoryBackgroundAutoRefresh([id],"restore-wf");
...
const combined=await computeP19X3HistoryBundlesAsync(p19Draws,id,{fast:fastMode,resolveTable:createWalkForwardTableResolver(id,p19Draws)});
...
setTimeout(()=>{
try{globalThis.X3NestedPro463?.clearCache?.();globalThis.X4Coverage577?.clearCache?.();}catch(_){}
},1200);
}
}catch(error){
...
function legacyRecordRelinkIds(profileIds=null){
const requested=Array.isArray(profileIds)?profileIds:(profileIds===null?(state.profiles||[]).map((_,id)=>id):[profileIds]);
return[...new Set(requested.map(Number).filter(id=>Number.isInteger(id)&&id>=0&&id<(state.profiles||[]).length))];
}
function markLegacyRecordRelinkPending(profileIds=null){
const pending=new Set((state.legacyRecordRelinkPendingProfileIds||[]).map(Number));
legacyRecordRelinkIds(profileIds).forEach(id=>pending.add(id));
state.legacyRecordRelinkPendingProfileIds=[...pending].sort((a,b)=>a-b);
return state.legacyRecordRelinkPendingProfileIds;
}
function importedBackupQuickReuseProof(validated){
...
return{eligible:true,ids,currentFingerprint,reason:"fingerprint-matched"};
}
function importedBackupVerifiedReuseProof(validated){
const quick=importedBackupQuickReuseProof(validated);
if(!quick.eligible)return quick;
const{ids,currentFingerprint}=quick;
const reused=[],invalid=[],results={};
for(const id of ids){
...
try{
setStatus("เริ่ม Turbo Rebuild แล้ว • เตรียม Cache…");
await nextUiFrame(0);
