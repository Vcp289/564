const BUILD = "72081aiprofilescopedrealtime";
...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72081\.css|pro-core-v72081\.js|app-v72081\.js|x3-pro-v72081\.js|manifest-v72081\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72081.json':
        url.pathname.endsWith('style-v72081.css')?'./style-v72081.css':
        url.pathname.endsWith('pro-core-v72081.js')?'./pro-core-v72081.js':
        url.pathname.endsWith('app-v72081.js')?'./app-v72081.js':'./x3-pro-v72081.js');
      if(hit) return hit;
