...
const APP_VERSION = "7.24.08-HISTORY-BOOT-AUTHORITY-PRO";
const APP_DISPLAY_VERSION = "V7.24.08 • Auto Route Pro";
const APP_BUILD_TAG = "72408historybootauthoritypro";
...
// V7.24.08 — DURABLE ROW JOURNAL PRO.
...
async function bootstrapPersistentState() {
  ...
  // V7.24.08 — HISTORY BOOT AUTHORITY PRO.
...
function scheduleCalculatorProfileRefresh(profileId = state.activeProfile) {
  ...
  requestAnimationFrame(() => setTimeout(() => {
    ...
    refreshCurrentView();
    // V7.24.08 AUTO ROUTE PRO: switching Profile restores synchronous mirrors first and
...
function autoRouteEvidenceReady(profileId){
  // V7.24.08 AUTO ROUTE PRO: Calculate readiness is no longer gated by X3 runtime.
...
function renderHistory() {
  ...
  // V7.24.08 COOL REFRESH: opening/rendering History is 100% read-only.
  ...
  const resultRows = visibleActualDraws
    .map(r => {
      ...
      // V7.24.08: a manually repaired strict-prior atomic row is itself durable evidence.
...
// V7.24.08 — Manual Refresh History (current Profile only).
...
async function hydrateApplicationAfterFirstPaint(){
  try{
    ...
    if(state.currentView==="weekly"){
      ...
      // V7.24.08 AUTO ROUTE PRO: release Calculate from X3 as soon as synchronous
...
// V7.24.08 iOS suspend guard: refresh only the tiny source+journal authority when the app
