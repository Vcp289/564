// V7.24.09 Hybrid Pro: instant durable Actual save + reliable old-style engine completion in serialized background.
(()=>{
  ...
  async function reliableSync(id,{affectedStartDate='',bootstrap=false,targetDrawId='',targetDate=''}={}){
    ...
    // V7.24.09 ROW-FIRST / PERCENT-LATER: a normal newest-day save is strictly incremental.
    ...
    if(!bootstrap){
      // V7.24.09 CONTINUOUS SAVE FIX: each queued save owns its exact row.
      ...
      // V7.24.09 ATOMIC ROW COMMIT PRO
  ...
  // V7.24.09: no automatic all-profile bootstrap on launch/import. Direct-source rendering is sufficient;
