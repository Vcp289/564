...
const APP_DISPLAY_VERSION = "V7.24.09 • Auto Route Authority Pro";
const APP_BUILD_TAG = "72409autorouteauthoritypro";
...
// V7.24.09 — DURABLE ROW JOURNAL PRO.
...
async function bootstrapPersistentState() {
  ...
  // V7.24.09 — HISTORY BOOT AUTHORITY PRO.
...
function scheduleCalculatorProfileRefresh(profileId = state.activeProfile) {
  ...
  requestAnimationFrame(() => setTimeout(() => {
    ...
    refreshCurrentView();
    // V7.24.09 AUTO ROUTE PRO: switching Profile restores synchronous mirrors first and
...
function autoRouteEvidenceReady(profileId){
  // V7.24.09 AUTO ROUTE PRO: Calculate readiness is no longer gated by X3 runtime.
...
function renderHistory() {
  ...
  // V7.24.09 COOL REFRESH: opening/rendering History is 100% read-only.
  ...
  const resultRows = visibleActualDraws
    .map(r => {
      ...
      // V7.24.09: a manually repaired strict-prior atomic row is itself durable evidence.
...
// V7.24.09 — Manual Refresh History (current Profile only).
...
async function hydrateApplicationAfterFirstPaint(){
  try{
    ...
    if(state.currentView==="weekly"){
      ...
      // V7.24.09 AUTO ROUTE PRO: release Calculate from X3 as soon as synchronous
...
// V7.24.09 iOS suspend guard: refresh only the tiny source+journal authority when the app
