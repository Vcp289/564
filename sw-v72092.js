const BUILD = "72092strictrebuildcontrol";
...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72092\.css|pro-core-v72092\.js|app-v72092\.js|x3-pro-v72092\.js|manifest-v72092\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72092.json':
        url.pathname.endsWith('style-v72092.css')?'./style-v72092.css':
        url.pathname.endsWith('pro-core-v72092.js')?'./pro-core-v72092.js':
        url.pathname.endsWith('app-v72092.js')?'./app-v72092.js':'./x3-pro-v72092.js');
      if(hit) return hit;
