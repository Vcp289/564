const BUILD = "72203routeaallengines";
...
self.addEventListener("fetch",event=>{
  ...
  const immutable = /(?:style-v72203\.css|pro-core-v72203\.js|app-v72203\.js|x3-pro-v72203\.js|manifest-v72203\.json)$/.test(url.pathname);
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72203.json':
        url.pathname.endsWith('style-v72203.css')?'./style-v72203.css':
        url.pathname.endsWith('pro-core-v72203.js')?'./pro-core-v72203.js':
        url.pathname.endsWith('app-v72203.js')?'./app-v72203.js':'./x3-pro-v72203.js');
      if(hit) return hit;
