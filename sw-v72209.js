...
self.addEventListener("fetch",event=>{
  ...
  if(immutable){
    event.respondWith(caches.open(CACHE).then(async cache=>{
      const hit=await cache.match(url.pathname.split('/').pop().startsWith('manifest')?'./manifest-v72209.json':
        url.pathname.endsWith('style-v72209.css')?'./style-v72209.css':
        url.pathname.endsWith('pro-core-v72209.js')?'./pro-core-v72209.js':
        url.pathname.endsWith('app-v72209.js')?'./app-v72209.js':'./x3-pro-v72209.js');
      if(hit) return hit;
